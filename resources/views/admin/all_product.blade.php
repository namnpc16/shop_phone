      ...
      <table class="table table-striped b-t b-light">
        ...
        @foreach($all_product as $key => $product)
          <tr>
            ...
            <td><img style="width: 100px" src="public/uploads/product/{{$product->product_image}}"></td>
            <td><span class="text-ellipsis"> {{ $product->product_price }}</span></td>
            <td>
              {{-- {{URL::to('/edit-product/'.$product->product_id)}} --}}
              <a href="edit-product/{{ $product->product_id }}" class="active styling-edit" ui-toggle-class="">
                <i class="fa fa-pencil-square-o text-success text-active"></i>
              </a>
              <a onclick="return confirm('Are you ready?')" href="{{URL::to('/delete-product/'.$product->product_id)}}" class="active styling-delete" ui-toggle-class="">
